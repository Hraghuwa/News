...
def generate_markdown_report(analysis_text: str, articles: list = None):
    ...
    Always includes raw article headlines so there's something to read.
    """
    ensure_reports_dir()
    ...
    # Section 1: AI Analysis (or error message)
    report_content += "## 🤖 AI Executive Analysis\n\n"
    report_content += analysis_text
    report_content += "\n\n---\n\n"

    # Section 2: Raw news headlines — always shown
    if articles:
        report_content += "## 📰 Today's Top Headlines\n\n"
        for i, a in enumerate(articles, 1):
            report_content += f"### {i}. {a['title']}\n"
            report_content += f"**Source:** {a['source']}\n\n"
            report_content += f"{a['summary']}\n\n"
            report_content += "---\n\n"
